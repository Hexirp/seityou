...
(** 覚え書き: 相同、ホモトピー、homotopy は、もともとの意味では、ある二つの
    ...
    ただし、型の相同論的な構造を単に相同と呼ぶ例もあるようで、よしなに解釈して
    ほしい。 *)


(** ** Equivalence *)

(** 等価性 (Equivalence) は相同的型理論 (Homotopy Type Theory) の中心的な
    概念である。「等価性」、その定義には三つの種類がある。

    一つ目は、互いに逆であるような [f : A -> B] と [g : B -> A] が存在する
    ことを要求することである。その条件は相同的に表される (up to homotopy) 。
    相同論的に語るならば、それらの相同には条件がさらに課されるべきである。
    それは、圏論の随伴に対する三角等式たち (triangle identities) の一つで
    ある。かように、この概念は随伴等価 (adjoint equivalence) と呼ばれる。

    他の三角等式は最初の三角等式から得られ、より高次元の一貫性 (coherence) の
    全ても同様であるため、一つだけを課するのは合理的であろう。さらに、互いに
    逆であるような二つの写像（もちろん逆であることは相同で表される）がある
    だけで三角等式を満たすように、それらの相同を修正できる。よって、随伴等価が
    得られる。この定理の証明は Equivalence を見よ。

    二つ目は、Vladimir Voevodsky による定義である。ある写像 [f : A -> B] の
    相同的維 (homotopy fiber) が可縮 (contractible) であるときとするもので、
    この概念は相同的全単射 (homotopy bijection) と呼ばれる。これは HoTT の
    ライブラリが作られた初期に使われていた。

    三つ目は、André Joyal による定義である。ある写像 [f : A -> B] が相同的
    右逆射と相同的左逆射を持つときとするもので、面白いことに右逆射と左逆射が
    同じである必要はない。この概念は相同的同型 (homotopy isomorphism) と
    呼ばれる。
...
(** [f] と [g] は [retr] と [sect] を通じて随伴的である。
    ...
    [A] の値を対象として [@paths A] を射とした圏（実際は可逆圏 (Groupoid) で
    ...
    ことになる。これは、普通の圏での随伴の定義を援用しただけである。

    ここで Equivalence の前説で書いた通り、実際にはただ一つの条件だけでよく
    （むしろ、一つだけの方が相同的な性質に関してよく振る舞う）、それは等式圏
    であるからである。この条件は [r] の両辺に [f] を右から合成したものと、
    そして [s] の両辺に [f] を左から合成したものを、さらに合成したとき、
    それが恒等関手になることだと定められる。式で表すと [f -> f g f -> f] と
    なる。

    この時 [s] を反転させて [s : pwpaths (compose g f) idmap] という風に[r] と
    綺麗に型をそろえてやる。すると [f g -> id] と [g f -> id] という風になり、
    これらに左右適切な向きから [f] を適用してできた [f g f -> f] が一致する
    という風に三角等式を表現できる。これは道が [p @ q^ = 1] から [p = q] へと
    移項できるからである。 *)
