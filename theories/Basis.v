...
Module Homotopical.
  ...
  (** 等価性 (Equivalence) は Homotopy Type Theory の中心的な概念である。
      「等価性」を定義する前に、私たちは [A] と [B] の二つの型が「同じ」と、
      どのようなときにみなすべきかについて様々な種類があることに注意して
      おかなければならない。

      一つ目は、互いに逆であるような [f : A -> B] と [g : B -> A] が存在し、
      それらが圏論の随伴に対する三角等値性 (triangle identities) の一つを
      満たす時である。ゆえに、この概念は随伴等価 (adjoint equivalence) と
      呼ばれる。

      要求しなかった他の三角等値性は、より高い全ての一貫性 (coherence) と共に
      最初の三角等値性から得られる。さらに、互いに逆であるような二つの写像が
      存在するだけで、それらを修正すれば三角等値性を得られる。
