...
(** 新しい帰納型を定義した時に、自動で帰納法の原理／除去子が定義されない
    ...
    除去子を自動生成される名前ではなく独自の名前で定義したいが、デフォルトでは
    そのようなことは不可能であるため。

    [induction] と [elim] 戦術は、対象となる型の除去子が、自動生成される
    デフォルトの名前を持っていることを前提にしている。そのため [induction] が
    使えなくなる欠点がある。

    代わりに [Local] でやれば設定は引き継がれないのだが、一貫させたいため
    この方法は取らなかった。自動生成される名前を別名とすれば戦術でも使える
    ようになる（と思う）が、そもそも [induction] 戦術などを使わない主義なため
    そうしなかった。

    もう一つの理由で、Prop 不使用という主義に反して Prop での帰納法が生成され
    るというのもある。この点は他の方法でも解決できて、今の方法を選択したのは
    自動生成される名前が気に入らないからである。
...
(** 参考文献:
    ...
    * https://github.com/coq/coq/blob/f4cf212efd98d01a6470ea7bfd1034d52e928906/theories/Init/Logic.v
